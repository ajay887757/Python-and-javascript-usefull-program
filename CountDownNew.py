...
class CountDown:
    def __init__(self, root):
        ...
        self.window.configure(bg='dark khaki')
...
if __name__ == "__main__":
    ...
    obj = CountDown(root)
    Label(root, font ='arial 15 bold', text = 'current time :', bg = 'dark khaki').place(x = 40 ,y = -4)
    def clock():
        clock_time = time.strftime('%H:%M:%S %p')
        curr_time.config(text = clock_time)
        curr_time.after(1000,clock)
    curr_time =Label(root, font ='arial 15 bold', text = '', fg = 'gray25' ,bg ='dark khaki')
    curr_time.place(x = 180 ,y = -4)
    clock()
    root.mainloop()